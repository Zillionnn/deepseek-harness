...
[`dsh-desktop`](../packages/desktop/app/README.md) 是同一 wire 上的一个载波：一个 Tauri 应用，把 web profile 作为隐藏子进程拉起，并在原生窗口中托管浏览器 surface，为产品提供双击、无控制台的入口（[Agent Note](../.agents/notes/implemented/architecture/2026-08-14-desktop-shell-tauri.zh.md)）。
